...
const T = {
  de: {
    ...
    downloadOriginal: 'Dokument herunterladen',
    translationButton: 'Ungarische Übersetzung',
    translationTitle: 'Ungarische Übersetzung',
    translationNote: 'Maschinelle Übersetzung, nur zur Information. Rechtlich verbindlich ist ausschließlich das deutsche Originaldokument, das Sie unterschreiben.',
    translationPage: (n) => `Seite ${n}`,
    translationDownload: 'Übersetzung als PDF',
    translationEmpty: 'Auf dieser Seite wurde kein Text gefunden.',
  ...
  hu: {
    ...
    downloadOriginal: 'Dokumentum letöltése',
    translationButton: 'Magyar fordítás',
    translationTitle: 'Magyar nyelvű fordítás',
    translationNote: 'Ez gépi fordítás, kizárólag tájékoztató jellegű. Jogilag kizárólag az aláírandó német nyelvű eredeti dokumentum az irányadó.',
    translationPage: (n) => `${n}. oldal`,
    translationDownload: 'Fordítás letöltése (PDF)',
    translationEmpty: 'Ezen az oldalon nem található szöveg.',
    translationLoading: 'A fordítás betöltése …',
  },
};
...
function renderShell() {
  app.innerHTML = `
    ...
    <div class="topbar">
      ...
      <div class="head-actions">
        ${doc.translation ? `<button class="btn small" id="trOpen">${t.translationButton}</button>` : ''}
        <a class="btn small" id="dlTop" href="/api/sign/${token}/file">${doc.completed ? t.download : t.downloadOriginal}</a>
      </div>
    </div>
    ${doc.banner ? `<div class="banner" id="banner"><div class="banner-inner">${esc(doc.banner)}</div></div>` : ''}
    </div>
  ...
  document.getElementById('dlTop').addEventListener('click', () => {
    const kind = doc.completed ? 'signed' : 'original';
    fetch(`/api/sign/${token}/downloaded?kind=${kind}`, { method: 'POST' }).catch(() => {});
  });

  const trOpen = document.getElementById('trOpen');
  if (trOpen) trOpen.addEventListener('click', openTranslation);
}
...
function setLanguage(lang) {
  ...
  if (dl) dl.textContent = doc.completed ? t.download : t.downloadOriginal;
  const tr = document.getElementById('trOpen');
  if (tr) tr.textContent = t.translationButton;
...
async function openTranslation() {
  const m = modal(`
    <div class="eyebrow">${t.translationButton}</div>
    <h2 style="margin:4px 0 14px">${esc(doc.title)}</h2>
    <div class="tr-note">${t.translationNote}</div>
    <div id="trBody"><div style="display:grid;place-items:center;padding:30px"><div class="spinner dark"></div>
      <div class="tiny muted" style="margin-top:10px">${t.translationLoading}</div></div></div>
    <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:6px">
      <button class="btn" id="trDl" disabled>${t.translationDownload}</button>
      <button class="btn primary" id="trClose">${t.close}</button>
    </div>`, { wide: true });
  m.el.querySelector('#trClose').addEventListener('click', m.close);

  try {
    if (!translationCache) {
      const res = await fetch(`/api/sign/${token}/translation`);
      if (!res.ok) throw new Error(t.error);
      translationCache = (await res.json()).pages || [];
    }
    const pages = translationCache;
    m.el.querySelector('#trBody').innerHTML = pages.map((text, i) => `
      <div class="tr-page">
        <h4>${t.translationPage(i + 1)}</h4>
        <p>${text ? esc(text) : `<span class="muted">${t.translationEmpty}</span>`}</p>
      </div>`).join('');
    const dl = m.el.querySelector('#trDl');
    dl.disabled = false;
    dl.addEventListener('click', async () => {
      dl.disabled = true;
      try {
        const bytes = await PDFTools.translationPdf({
          title: doc.title + ' — ' + T.hu.translationTitle,
          pages,
          note: T.hu.translationNote + '  /  ' + T.de.translationNote,
          pageLabel: T.hu.translationPage,
        });
        PDFTools.download(bytes, (doc.title || 'Dokument').replace(/[^\p{L}\p{N} _.-]/gu, '').trim().slice(0, 50) + ' – fordítás.pdf');
        fetch(`/api/sign/${token}/downloaded?kind=translation`, { method: 'POST' }).catch(() => {});
      } catch (err) { toast(t.error, 'err'); }
      dl.disabled = false;
    });
  } catch (err) {
    m.el.querySelector('#trBody').innerHTML = `<p class="small" style="color:var(--red)">${t.error}</p>`;
  }
}
...
async function submit() {
  ...
  try {
    ...
    document.getElementById('dlTop').textContent = t.download;
    showThanks();
  } catch (err) {
